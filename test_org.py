    ...
    if httpMethod == getMethod and path == fetchdatapath:
        ...
        try:
            ...
            for table in table_names:
                ...
                if existing_item:
                    #if 'phone_updated' in existing_item and existing_item['phone_updated']['S'] == phone_no:
                    existing_phone_no = existing_item.get('phone_updated', {}).get('N')
                    logger.info(f"existing phone no is {existing_phone_no}")
                    if existing_phone_no:
                        ...
                        logger.info(f"preprocessed phone no is {preprocessed_phone_no}")
                        if preprocessed_existing_phone_no == preprocessed_phone_no:
                        
                            # POEPhone matches, update the item with the provided attributes
                    ...
                    # Set the flag to True if the file_number is found in any table
            ...
            # If no matching item is found in any table, return an appropriate response
            logger.error('Matching item not found in any table')
            return {
                'statusCode': 404,
                'body': 'Matching item not found in any table'
            }
        except ValueError:
